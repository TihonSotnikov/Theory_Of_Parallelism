...
public:
    ...
    void wait()
    {
        ...
        {
            cv.wait(lock, [this, local_generation] {
                return local_generation != generation;
            });
        }
    }
...
inline void init_one_chunk(
    const size_t start_idx,
    const size_t end_idx,
    const size_t n,
    double* RESTRICT_PTR mat,
    double* RESTRICT_PTR vec,
...
inline void process_one_chunk(
    const size_t start_idx,
    const size_t end_idx,
    const size_t n,
    const double* RESTRICT_PTR mat,
    const double* RESTRICT_PTR vec,
    double* RESTRICT_PTR res)
{
    assert(start_idx <= end_idx && end_idx <= n && "Bounds must be in range [0, n)");
...
inline void worker_routine(
    const size_t runs_num,
    const size_t start_idx,
    const size_t end_idx,
    const size_t n,
    double* mat,
    ...
    const size_t runs_num = 1)
{
    ...
    if (threads_num == 0)
        throw std::invalid_argument("Threads number must be > 0");
    ...
    for (size_t thread = 0; thread < threads_num; ++thread)
    {
        ...
        threads.emplace_back(
            [=, &start_barrier, &end_barrier,
             mat_ptr = mat.get(), vec_ptr = vec.get(), res_ptr = res.get()]
            {
                try
                {
    ...
    std::cout << "\n[ RESULTS ]\n"
              ...
              << "  Avg Time   : " << std::chrono::duration<double>(total_time).count() / runs_num << " s\n\n";
#endif
